...
現在のPC向けUI・操作設計は [2026-09-13 操作整理の記録](thermal-flow-ux.md) を参照。本書のUI構成・ロードマップは初期設計時の記録。
...
**② カラー LUT（このツールの主役）**
- 編集可能グラデーションストップ（位置・色・**硬さ**）。現行実装は最大64個、4096サンプル・RGB各16bitのLUT（上位／下位バイトの2行テクスチャ）を使用。旧256×1・8bit LUTは2026-09-13に置換
